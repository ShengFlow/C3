...
private:
     ...
     * @details env `C3_PARTITION_AB=1` 门控。分别编译并计时:
     *          A = 整图一个内核; B = partitionGraph 切出的多个子图各一内核。
     *          采用交错多轮配对测量(每轮 A/B 各执行若干次, 取中位数 + 胜负计数),
     *          避免单次测量受热降频/调度抖动支配。
     *          假输入填充规则: 原图输入占位节点填 0.5; 子图外部输入若来源于原图 Const
     *          节点则填该常量真实值(Const 属 LEAF, 在子图中被暴露为外部输入);
     *          若存在既非图输入又非 Const 的子图外部输入, 则数值对比不可信, 直接跳过。
     *          同时校验数值正确性: A 的图输出 vs B 各子图输出按原图输出 id 对齐比较。
     *          不改变任何既有执行路径。
